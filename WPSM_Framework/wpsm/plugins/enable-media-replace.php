 ...
 *
 * @author      Mans Jonasson  <http://www.mansjonasson.se>
 * @copyright   Mans Jonasson 13 sep 2010
 * @package     wordpress
 * @subpackage  enable-media-replace
 *
 */

define('EMR_DIRECTORY', dirname(__FILE__) . '/enable-media-replace');
...
function emr_options() {
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] == 'media_replace' ) {
		...
		if ( array_key_exists("attachment_id", $_GET) && (int) $_GET["attachment_id"] > 0) {
			include(EMR_DIRECTORY . '/popup.php');
		}
	}
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] == 'media_replace_upload' ) {
		...
		require_once(EMR_DIRECTORY . '/upload.php');
	}
}
