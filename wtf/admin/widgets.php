...
function wtf_widget_page() {
    $saved = false;
    if ($_REQUEST['action'] == 'save') {
        $wp_cal = ($_POST['wp_calendar'] == 'true') ? true : false;
        $gcal = ($_POST['gcal'] == 'true') ? true : false;
        $twitter = ($_POST['twitter'] == 'true') ? true : false;
        update_option('wtf-widgets-wp-calendar', $wp_cal);
        update_option('wtf-widgets-google-calendar', $gcal);
        update_option('wtf-widgets-twitter', $twitter);
    ...
    <?php if ($saved) { ?><div class="updated fade" id="message"><p><strong>Settings saved.</strong></p></div><?php } ?>
            <p>These widgets are theme specific and will only apply when the current theme (<strong><?php echo WTF_THEME_NAME; ?></strong>) is enabled.</p><br />
            <!-- START SECTION -->
            <div class="section">
    ...
    if ((bool) get_option('wtf-widgets-twitter') == true) {
        echo ' checked="checked"';
    }
    ?> /> Yes</label><br />
                        <label for="twitter_no"><input type="radio" value="false" id="twitter_no" name="twitter"<?php
    if ((bool) get_option('wtf-widgets-twitter') == false) {
        echo ' checked="checked"';
    }
    ?> /> No</label>
                    </fieldset>
    ...
    if ((bool) get_option('wtf-widgets-google-calendar') == true) {
        echo ' checked="checked"';
    }
    ?> /> Yes</label><br />
                        <label for="gcal_no"><input type="radio" value="false" id="gcal_no" name="gcal"<?php
    if ((bool) get_option('wtf-widgets-google-calendar') == false) {
        echo ' checked="checked"';
    }
    ?> /> No</label>
                    </fieldset>
    ...
    if ((bool) get_option('wtf-widgets-wp-calendar') == true) {
        echo ' checked="checked"';
    }
    ?> /> Yes</label><br />
                        <label for="wp_calendar_no"><input type="radio" value="false" id="wp_calendar_no" name="wp_calendar"<?php
    if ((bool) get_option('wtf-widgets-wp-calendar') == false) {
        echo ' checked="checked"';
    }
    ?> /> No</label>
                    </fieldset>
...
function wtf_widget_main_page() {
    ?>
    <div class="wrap">
        <div id="icon-themes" class="icon32"></div> <h2><?php echo WTF_THEME_NAME; ?></h2>
        <p>As you enable widgets in the menu above ("<?php echo WTF_THEME_NAME; ?>"->"Widgets"), their options pages will show up here.</p>
    </div>
    <?php
} //end wtf_widget_main_page