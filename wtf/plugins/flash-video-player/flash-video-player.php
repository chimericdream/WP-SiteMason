...
if (!is_admin()) {
    $swfprotocol = is_ssl() ? 'https' : 'http';
    wp_register_script('swfobject', "{$swfprotocol}://ajax.googleapis.com/ajax/libs/swfobject/2.2/swfobject.js", NULL, NULL);
...
function FlashVideoPlayerPlugin_renderplayer($tag_string) {	
	...
	if ( array_key_exists('image', $inline_options) ) {
		...
		if ($saved_options['File Properties']['image']['v'] == '') {
			...
			$rss_output .= '<img src="' . TEMPLATEPATH . '/plugins/flash-video-player/default_video_player.gif" />';
		} else {
			$rss_output .= '<img src="' . $saved_options['File Properties']['image']['v'] . '" />';
		}
	}
			...
			if ( $value['v'] != '' && $value['v'] != 'undefined') {
				...
				if($key == 'skin') {
					//if($value['v'] != 'undefined') {
						$flashvars[] = "\n\t'" . $key . "' : '" . TEMPLATEPATH . "/plugins/flash-video-player/skins/" . $value['v'] . '/' . trim($value['v']) . ".swf'";
					//}
				} else {
					$flashvars[] = "\n\t'" . $key . "' : '" . trim($value['v']) . "'";
				}	
	...
	$output .=  $flashvars;
	$output .= "\n" . ' };';
	$output .= "\n" . 'swfobject.embedSWF("' . TEMPLATEPATH .'/plugins/flash-video-player/mediaplayer/player.swf", "video' . $videoid . '", "' . $saved_options['Video Size']['width']['v'] . '", "' . $saved_options['Video Size']['height']['v'] . '", "9.0.0","' . TEMPLATEPATH . '/plugins/flash-video-player/mediaplayer/expressinstall.swf", flashvars, params, attributes);';
	$output .= "\n" . '</script>';
